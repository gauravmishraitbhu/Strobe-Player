...
package org.osmf.player.chrome.widgets
{
	...
	{	
		...
		public function showInfo():void
		{
			...
			videoInfoOverlay = new Sprite;
			videoInfoOverlay.graphics.beginFill(0x1A1A1A,0.6);
			videoInfoOverlay.graphics.drawRect(0,0,300,200);
			videoInfoOverlay.graphics.endFill();
			...
			textField.width = videoInfoOverlay.width - 10;
			textField.height = videoInfoOverlay.height-10;
			textField.x = textField.y = 5;
			videoInfoOverlay.addChild(textField);
			var tFormat:TextFormat = new TextFormat();
			tFormat.size = 12;
			tFormat.bold = true;
			tFormat.font = "Arial"
			tFormat.color = 0x00AA00;
			textField.defaultTextFormat = tFormat;
			
			
			closeButton = new Sprite;
			var xTF:TextField = new TextField;
			
			xTF.defaultTextFormat = tFormat;
			xTF.text = "[x]";
			xTF.height = xTF.width = 25;
			closeButton.graphics.beginFill(0x1A1A1A,0.3);
			closeButton.graphics.drawRect(0,0,xTF.width,xTF.height);
			closeButton.graphics.endFill();
			closeButton.addChild(xTF);
			closeButton.x = videoInfoOverlay.width - closeButton.width ;
			closeButton.y = 10;
			videoInfoOverlay.addChild(closeButton);
			closeButton.addEventListener(MouseEvent.CLICK,onCloseButtonClick);
			onTimer(null);
		}
			...
			if (media && media.hasTrait(MediaTraitType.LOAD))
			{
				...
				if (loadTrait)
				{
					...
					if (netStream)
					{
						...
						videoInfo += "bitrate \t"
							+ Math.round(netStream.info.playbackBytesPerSecond*8)/1000
							+ "kbps\n";
					}
				}
			}
