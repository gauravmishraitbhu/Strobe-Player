	...
	public class ScrubBar extends Widget
	{
		...
		private function updateScrubberPosition(event:Event = null):void
		{
			...
			if (timeTrait != null && timeTrait.duration)
			{
				...
				var bufferTrait:BufferTrait = media ? media.getTrait(MediaTraitType.BUFFER) as BufferTrait : null;
				...
				{				
					...
					if (loadTrait)
					{
						...
						if(loadTrait.bytesTotal && loadTrait.bytesLoaded){
							percentLoaded = (Math.min(1.0, loadTrait.bytesLoaded / loadTrait.bytesTotal)) 
						}else if(bufferTrait != null){
							percentLoaded = (timeTrait.currentTime + bufferTrait.bufferLength)/duration ;
						...
						percentLoaded = Math.min(percentLoaded,1.0);
