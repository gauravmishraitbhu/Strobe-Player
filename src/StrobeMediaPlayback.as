...
{	
	...
	public class StrobeMediaPlayback extends Sprite
	{
		...
		private var _errorProcessed:Boolean = false;
		...
		public function initialize(parameters:Object, stage:Stage, loaderInfo:LoaderInfo, pluginHostWhitelist:Array):void
		{
			...
			player.addEventListener(PlayEvent.LIVE_STALL,onStall);
		...
		private function onAddedToStage(event:Event):void
		{
			...
			if(ExternalInterface.available){
				ExternalInterface.call("onAddedToStage");
			}
		}
		...
		private function onStall(e:Event):void{
			CONFIG::LOGGING{
				logger.info("Error","stall detected");
			}
			retry();
		}
		
		private function retryMedia():void {
		...
		private function retry():void{
			if(alert!=null){
				alert.alert("Error","Retrying connection...");
			}
			setTimeout(retryMedia,3000);
		}
		
		private function onMediaError(event:MediaErrorEvent):void
		{
			var customMessage:String = "Stream is currently offline...\n Please try again after sometime"
				
			if(ExternalInterface.available){
				ExternalInterface.call("console.log","error playing stream--image-"+event.error.message);
			}
			//retry only for live
			if(!configuration.playingVOD){
				setTimeout(retry,12000);
			}else{
				//for vods change msg
				customMessage = "Video not found...";
			}
			...
			CONFIG::FLASH_10_1
				...
				{
					trace("Error:", message); 
					//to avoid infinite loops incase the process poster function
					//starts erroring 
					if(ExternalInterface.available){
						ExternalInterface.call("console.log","error playing stream--image-"+message);
					}
					if(!_errorProcessed){
						if(ExternalInterface.available){
							ExternalInterface.call("console.log",message);
						}
						if(configuration.endOfVideoOverlay !=null){
							processPoster(configuration.endOfVideoOverlay);
						}
