	...
	public class HTTPStreamSource implements IHTTPStreamSource, IHTTPStreamHandler
	{
		...
		public function seek(offset:Number):void
		{
			...
			if (_seekTarget < 0 )
			{
				if (_dvrInfo != null)
				{
					_seekTarget = int(_dvrInfo.startTime + _dvrInfo.curLength - OSMFSettings.hdsDVRLiveOffset);
				}
				else
				{
//					trace("seek target was--"+_seekTarget);
//					trace("isLive--"+_isLive);
					if (_isLive)
					{
						_seekTarget = int(_offset);
					}
					else
					{
						_seekTarget = 0;
...
//			trace("QQQQ---seektarget--"+_seekTarget);
//			trace("YYYYY---offset---"+offset);
			CONFIG::LOGGING
			{			
